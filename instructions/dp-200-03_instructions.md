...
4. In this code block, replace the **application-id**, **authentication-key (Use the value of DLAccessxx secret, not the ID of the secret that you pasted earlier in the notepad)**, **tenant-id**, and **storage-account-name** placeholder values in this code block* with the values that you collected earlier and are held in notepad.
